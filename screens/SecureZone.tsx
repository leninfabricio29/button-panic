...
type EntityType = "police" | "ambulance" | "fire" | "security_private" | "other";
...
const SecureZone = () => {
  ...
  const filterOptions: FilterOption[] = [
    ...
    { type: "police", label: "Policía", icon: "shield-outline", color: "#3B82F6" },
    { type: "ambulance", label: "Ambulancia", icon: "medical-outline", color: "#EF4444" },
    { type: "fire", label: "Bomberos", icon: "flame-outline", color: "#F97316" },
    { type: "security_private", label: "Seguridad", icon: "lock-closed-outline", color: "#8B5CF6" },
    { type: "other", label: "Otros", icon: "ellipsis-horizontal-outline", color: "#6B7280" },
  ];
  ...
  useEffect(() => {
    if (selectedFilter === "all") {
      ...
      setFilteredEntities(entities.filter((entity) => entity.type === selectedFilter));
    }
  ...
  const handleSubscribeToggle = async (entityId: string, isSubscribed: boolean) => {
    setLoadingEntityId(entityId);
    try {
      if (!isSubscribed) {
        await usersService.suscribeEntity({ entityId, userId: user._id });
      } else {
      ...
      if (error.response?.status === 403 && error.response.data?.message === "Límite de suscripciones alcanzado.") {
        Alert.alert(
          "Límite alcanzado",
        ...
        Alert.alert("Error", "No se pudo completar la operación.", [{ text: "OK" }]);
      }
    } finally {
  ...
  const renderFilterButton = ({ item }: { item: FilterOption }) => (
    <TouchableOpacity
      style={[
        styles.filterButton,
        {
          backgroundColor: selectedFilter === item.type ? item.color : (isDark ? "#1F2937" : "#F3F4F6"),
          borderColor: selectedFilter === item.type ? item.color : (isDark ? "#374151" : "#E5E7EB"),
        },
      ]}
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          <View style={styles.entityInfo}>
            <View style={[styles.iconContainer, { backgroundColor: entityColor + "20" }]}>
              <Ionicons name={entityIcon} size={24} color={entityColor} />
            </View>
            <View style={styles.textInfo}>
              <Text style={styles.title}>{item.name}</Text>
              <Text style={styles.type}>
                {filterOptions.find((f) => f.type === item.type)?.label || item.type}
              </Text>
              {item.address && (
          ...
          <View style={styles.subscribersInfo}>
            ...
            <Text style={styles.subscribersCount}>{item.users_suscribed.length}</Text>
          </View>
        </View>
      ...
      <View style={styles.headerInfo}>
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoText}>
            Suscríbete a las zonas seguras para recibir notificaciones y actualizaciones importantes de emergencia en tu área.
          </Text>
        </View>
      </View>
...
function createStyles(isDark: boolean) {
  return StyleSheet.create({
    ...
    infoCard: {
      backgroundColor: isDark ? "#1E40AF20" : "#EBF8FF", // sombra azul muy clara en oscuro
      borderRadius: 12,
      padding: 16,
      flexDirection: "row",
      ...
      gap: 12,
      borderLeftWidth: 4,
      borderLeftColor: isDark ? "#60A5FA" : "#3B82F6", // azul más claro en oscuro
    },
    infoText: {
      flex: 1,
      fontSize: 14,
      color: isDark ? "#BFDBFE" : "#1E40AF", // texto azul claro en oscuro
      lineHeight: 20,
    },
    filtersContainer: {
      paddingHorizontal: 16,
    ...
    type: {
      fontSize: 14,
      fontWeight: "500",
      color: isDark ? "#94A3B8" : "#6B7280", // color más tenue en modo oscuro
    },
    addressContainer: {
      flexDirection: "row",
      alignItems: "center",
    ...
    subscribersInfo: {
      ...
      alignItems: "center",
      gap: 4,
      backgroundColor: isDark ? "#374151" : "#F3F4F6", // gris oscuro para modo oscuro
      paddingHorizontal: 8,
      paddingVertical: 4,
      borderRadius: 12,
    },
